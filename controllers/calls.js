...
export const makeCall = async (req, res) => {
    ...
    receiverStatusRef.get().then(async (snap) => {
        ...
        const callToken = await (await axios.get(`https://agora-token-server-5ne6.onrender.com/rtc/${callId}/publisher/uid/0/`,{timeout:1000*40})).data.rtcToken;
        callRequestRef.set({ callerId: user.fuid, callerName: user.name, callState: 'default', callId, callToken, callType });
        setTimeout(async () => {
